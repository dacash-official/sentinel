...
sentinel_cfg = DACashConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_dacashd_proto_version_with_sentinel_ping = 70207


def get_dacash_conf():
    if sys.platform == 'win32':
        dacash_conf = os.path.join(os.getenv('APPDATA'), "DACashCore/dacash.conf")
    else:
        home = os.environ.get('HOME')

        dacash_conf = os.path.join(home, ".dacashcore/dacash.conf")
        if sys.platform == 'darwin':
            dacash_conf = os.path.join(home, "Library/Application Support/DACashCore/dacash.conf")

    dacash_conf = sentinel_cfg.get('dacash_conf', dacash_conf)
