...
class DACashConfig():
    ...
    def slurp_config_file(self, filename):
        # read dacash.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is dacash.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from dacash.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard DACash defaults...
        default_port = 56001 if (network == 'mainnet') else 56101

        # use default port for network if not specified in dacash.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from dacash.conf
