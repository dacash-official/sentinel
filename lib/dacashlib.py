...
def is_valid_dacash_address(address, network='mainnet'):
    ...
    dacash_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid DACash address
        return False

    if (address_version != dacash_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.dacashd_serialise())
...
def SHIM_serialise_for_dacashd(sentinel_hex):
    ...
    # superblock => "trigger" in dacashd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # dacashd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    dacashd_hex = serialise(obj)
    return dacashd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('dacash.conf')
