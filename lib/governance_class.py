...
class GovernanceClass(object):
    ...
    def vote(self, dacashd, signal, outcome):
        return self.go.vote(dacashd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, dacashd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(dacashd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(dacashd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import dacashlib
        obj_data = dacashlib.SHIM_serialise_for_dacashd(self.serialise())
    ...
    def dacashd_serialise(self):
        import dacashlib
        return dacashlib.SHIM_serialise_for_dacashd(self.serialise())
