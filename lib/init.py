...
def has_dacash_conf():
    ...
    valid_dacash_conf = False

    # ensure dacash_conf exists & readable
    #
    # if not, print a message stating that DACash Core must be installed and
    # configured, including JSONRPC access in dacash.conf
    try:
        f = io.open(config.dacash_conf)
        valid_dacash_conf = True
    except IOError as e:
        print(e)

    return valid_dacash_conf
...
def main():
    ...
    if not has_dacash_conf():
        print("DACash Core must be installed and configured, including JSONRPC access in dacash.conf")
        sys.exit(1)
