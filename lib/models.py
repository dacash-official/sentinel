...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, dacashd):
        golist = dacashd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_dacashd(dacashd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_dacashd(self, dacashd, rec):
        import decimal
        import dacashlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from dacashd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from dacashd, with every run
        ...
        # get/create, then sync payment amounts, etc. from dacashd - DACashd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(dacashd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from dacashd! %s" % e)
            govobj.vote_delete(dacashd)
            return (govobj, None)
    ...
    def vote_delete(self, dacashd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(dacashd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, dacashd, signal, outcome):
        import dacashlib
        ...
        output = dacashd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = dacashlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(dacashd, signal)

    def sync_network_vote(self, dacashd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = dacashd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = DACASHD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 dacash addr, non-multisig
            if not dacashlib.is_valid_dacash_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid DACash address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # DACash Core restricts proposals to 512 bytes max
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = DACASHD_GOVOBJ_TYPES['superblock']
    ...
    def hash(self):
        import dacashlib
        return dacashlib.hashit(self.serialise())
