...
def test_valid_dacash_address():
    from dacashlib import is_valid_dacash_address

    main = valid_dacash_address()
    test = valid_dacash_address('testnet')

    assert is_valid_dacash_address(main) is True
    assert is_valid_dacash_address(main, 'mainnet') is True
    assert is_valid_dacash_address(main, 'testnet') is False

    assert is_valid_dacash_address(test) is False
    assert is_valid_dacash_address(test, 'mainnet') is False
    assert is_valid_dacash_address(test, 'testnet') is True


def test_invalid_dacash_address():
    from dacashlib import is_valid_dacash_address

    main = invalid_dacash_address()
    test = invalid_dacash_address('testnet')

    assert is_valid_dacash_address(main) is False
    assert is_valid_dacash_address(main, 'mainnet') is False
    assert is_valid_dacash_address(main, 'testnet') is False

    assert is_valid_dacash_address(test) is False
    assert is_valid_dacash_address(test, 'mainnet') is False
    assert is_valid_dacash_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from dacashlib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % dacashlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(dacashlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(dacashlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(dacashlib.blocks_to_seconds(16616)) == 2612035
